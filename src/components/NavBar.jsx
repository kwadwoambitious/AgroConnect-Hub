...
import ProfileModal from "./Modals/ProfileModal"; // Assuming you have this component
import CartModal from "./Modals/CartModal"; // Assuming you have this component
...
const NavBar = ({ logoImage, textColor }) => {
  ...
  const [userInitials, setUserInitials] = useState(null); // New state for user initials
  ...
  useEffect(() => {
    // Check for user initials in local storage
    const storedUserInitials = localStorage.getItem("userInitials");
    if (storedUserInitials) {
  ...
  return (
    <div className="flex justify-center">
      <div className="w-[100%] px-5 lg:px-20 mx-auto flex items-center justify-between 2xl:px-24 z-[80] fixed top-0 md:mt-0 bg-[#FFFFFF] shadow-[0px_19px_26px_-20px_rgba(0,0,0,0.1)] py-6 h-[90px]">
        <div className="flex items-center gap-x-2">
          ...
          <Link to="/" className="flex items-center">
            ...
            />
            <h2 className="text-[13px] font-extrabold leading-none tracking-tight text-[#2E982D] md:text-xl lg:text-[22px]">
              AgroConnect Hub
            </h2>
          </Link>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          {userInitials ? (
            <div className="relative" ref={profileRef}>
              ...
              >
                <p className="bg-[#2e982d] text-white w-7 h-7 md:w-9 md:h-9 rounded-full flex items-center justify-center p-2 font-bold hover:border text-[12px] md:text-[13px] hover:border-gray-800">
                  {userInitials}
                </p>
              </div>
            ...
            <Link
              to="/login"
              className="items-center ml-3 lg:ml-10 font-semibold text-[12px] lg:text-[13px] text-white bg-[#2E982D] hover:bg-[#1e6a1e] hover:shadow-2xl transition duration-300 ease-in-out py-1 lg:py-2 px-2 lg:px-6 rounded-md"
            >
              Login
            </Link>
          )}

  ...
  return (
    <div className="fixed inset-0 bg-black backdrop-blur-md bg-opacity-50 flex items-center justify-center z-[90]">
      <div className="bg-white p-6 rounded-lg shadow-lg w-11/12 md:w-[30%]">
        ...
        <p className="text-gray-500">
          <span className="font-medium">Reviews:</span> {product.ratingsAverage} ({product.ratingsQuantity})
        </p>
      </div>
    </div>
  );
...
const AllProducts = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get('https://api-agroconnect.onrender.com/api/v1/products');
            ...
            {products.map((product, index) => (
              ...
              >
                ...
                <div className="px-4 py-4">
                  ...
                  <p className="text-gray-500 text-[12px] md:text-[14px]">
                    <span className="font-medium">Ratings:</span> {product.ratingsAverage} ({product.ratingsQuantity})
                  </p>
                  <button
